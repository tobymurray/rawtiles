...
- [x] `golden-png-to-pack-1tile.rawtiles` — 1 tile carrying the § 14.4 quantiser test vector; module self-tests § 9.1.1 against § 14.4's published RGB888↔ABGR2222 pair. PNG decode step is writer-implementation-specific per § 14.1 and out of scope; the corpus pins the post-decode RGB888 → quantiser → pack path.
- [x] `golden-png-to-pack-5tiles.rawtiles` — multi-tile companion at z=0..1 (1 + 4 tiles); z=0 carries § 14.4 vector, z=1 carries solid red/green/blue/white. Exercises the multi-tile quantiser path that the 1-tile fixture cannot.
...
- [x] `golden-zmax.rawtiles` — tile at `z = 23` (max legal zoom); pins both the i64-exact lon path and the libm-dependent atan/sinh lat path of § 4.9
- [x] `golden-canonical-uuid.rawtiles` — pack whose `pack_uuid` is derived per Appendix A; module self-tests reproduce § A.5's worked example end-to-end before emitting bytes
...
- [x] `neg-14a-offset-misaligned.rawtiles` — entry `offset` not 4-aligned [#14a]
- [x] `neg-14b-offset-below-blob.rawtiles` — entry `offset < tile_blob_start` [#14b]
- [x] `neg-14c-offset-past-ext.rawtiles` — entry `offset ≥ extensions_offset` [#14c]
- [x] `neg-14d-length-overrun.rawtiles` — entry `length > extensions_offset − offset` [#14d]
